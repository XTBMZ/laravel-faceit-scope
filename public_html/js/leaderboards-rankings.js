/**
 * Script optimisé pour la page des classements FACEIT Rankings - Faceit Scope
 * Utilise les vraies APIs FACEIT Rankings
 */
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🏆 Initialisation des classements FACEIT Rankings');
...
function setupEventListeners() {
    ...
    if (regionSelect) {
        regionSelect.addEventListener('change', function() {
            ...
            console.log('🌍 Changement de région:', currentRegion);
    ...
    if (countrySelect) {
        countrySelect.addEventListener('change', function() {
            ...
            console.log('🏴 Changement de pays:', currentCountry);
            debouncedLoadLeaderboard();
        });
    }
    ...
    if (limitSelect) {
        limitSelect.addEventListener('change', function() {
            ...
            console.log('📊 Changement de limite:', currentLimit);
            debouncedLoadLeaderboard();
        });
    }
...
async function loadLeaderboard(forceRefresh = false) {
    console.log('📊 Chargement du classement FACEIT Rankings:', {
    ...
    try {
        ...
        console.time('⚡ API FACEIT Rankings');
        console.log('🌐 Requête vers:', `/api/leaderboard?${params}`);
        ...
        console.timeEnd('⚡ API FACEIT Rankings');
        ...
        showError(`Erreur lors du chargement du classement: ${error.message}`);
        hideLoading();
        throw error;
    }
}
...
async function loadRegionStats(forceRefresh = false) {
    try {
        ...
        console.log('📊 Chargement stats région:', currentRegion);
        ...
        if (response.ok) {
            ...
            if (data.success) {
                ...
                console.log('✅ Stats région mises à jour:', data.data);
            }
        }
    } catch (error) {
...
function updateRegionStats(stats) {
    ...
    if (topLevelEl && stats.top_level) {
        topLevelEl.textContent = `Niveau ${stats.top_level}`;
    }
}
...
async function searchPlayer(playerName) {
    console.log('🔍 Recherche joueur:', playerName);
    ...
    const cacheKey = `search_${playerName}_${currentRegion}_${currentCountry}`;
    ...
    searchResult.innerHTML = `
        <div class="flex items-center justify-center py-4 bg-faceit-elevated/50 rounded-lg">
            ...
            <span class="text-gray-300">Recherche de ${playerName} dans le classement ${currentRegion}...</span>
        </div>
    `;
    ...
    try {
        ...
        console.time('🔍 Recherche joueur API');
        ...
        console.timeEnd('🔍 Recherche joueur API');
...
function displayPlayerSearchResult(player) {
    ...
    console.log('🎯 Affichage résultat recherche:', player);
...
console.log('🏆 Script des classements FACEIT Rankings chargé avec succès!');